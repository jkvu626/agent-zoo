...
export async function registerRoutes(app: FastifyInstance, store: AgentStore): Promise<void> {
  // List agents
  ...
  app.get<{ Params: AgentIdParams }>(
    "/api/agents/:id",
    async (req: FastifyRequest<{ Params: AgentIdParams }>, reply: FastifyReply) => {
  ...
  app.post<{ Body: Omit<Agent, "id"> }>(
    "/api/agents",
    async (req: FastifyRequest<{ Body: Omit<Agent, "id"> }>, reply: FastifyReply) => {
      const body = req.body ?? {};
  ...
  app.put<{ Params: AgentIdParams; Body: Partial<Omit<Agent, "id">> }>(
    "/api/agents/:id",
    async (
      req: FastifyRequest<{ Params: AgentIdParams; Body: Partial<Omit<Agent, "id">> }>,
      reply: FastifyReply
    ) => {
  ...
  app.put<{ Body: { currentAgentId: string | null } }>(
    "/api/current",
    async (req: FastifyRequest<{ Body: { currentAgentId: string | null } }>, reply: FastifyReply) => {
